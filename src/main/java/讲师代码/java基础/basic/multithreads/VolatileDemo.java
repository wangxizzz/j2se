        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            Thread thread = new Thread(() -> {                
                for (int j = 0; j < 1000; j++) {
                    state = ThreadLocalRandom.current().nextLong() % (threadIndex + 1);
//                    logger.trace("{}th thread {}th number state:{}", threadIndex, j, state);
                }               
            });
            threads[i] = thread;
            thread.start();
        ...
        try {
            ...
            logger.info("sub thread's jobs are done, state:{}", state);
